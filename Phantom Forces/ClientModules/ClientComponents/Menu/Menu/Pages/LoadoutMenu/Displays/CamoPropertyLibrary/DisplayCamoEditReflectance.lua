...
local function u6(p1)
	return (p1.BrickProperties or {}).Reflectance or ((u1.getSkinDataset(p1.Name).BrickPreset or {}).Reflectance or 0);
end;
local u7 = shared.require("ActiveLoadoutEvents");
function v1.activate(p2, p3, p4)
	local v7 = l__Templates__2.DisplayEditSlider:Clone();
	v7.Parent = p2;
	u3.setText(v7.TitleEdit, string.upper("Editing Reflectance"));
	u3.setText(v7.TitleValue, "Reflectance:");
	...
	p4:add(v9.onDragEnded:connect(function(p5)
		...
		u5.changeCamoProperty(v8, p3, "BrickProperties", "Reflectance", p5);
	end));
	p4:add(v9.onChanged:connect(function(p6, p7)
		...
		if p7 then
			u5.changeCamoProperty(v8, p3, "BrickProperties", "Reflectance", p6);
		end;
	end));
	local function v10()
...
function v1.init(p8)
	u3.setText(p8, string.upper("Reflectance"));
end;
return v1;
