...
local function u6(p1)
	return (p1.TextureProperties or {}).StudsPerTileV or ((u1.getSkinDataset(p1.Name).TexturePreset or {}).StudsPerTileV or 0);
end;
local u7 = shared.require("ActiveLoadoutEvents");
function v1.activate(p2, p3, p4)
	local v7 = l__Templates__2.DisplayEditSlider:Clone();
	v7.Parent = p2;
	u3.setText(v7.TitleEdit, string.upper("Editing StudsPerTileV"));
	u3.setText(v7.TitleValue, "StudsPerTileV:");
	...
	p4:add(v9.onDragEnded:connect(function(p5)
		...
		u5.changeCamoProperty(v8, p3, "TextureProperties", "StudsPerTileV", p5);
	end));
	p4:add(v9.onChanged:connect(function(p6, p7)
		if not v2.getActiveCamoData(p3) then
			return;
		end;
		v7.TextBox.Text = string.format("%0.3f", math.round(p6 * 1000) / 1000);
		if p7 then
			u5.changeCamoProperty(v8, p3, "TextureProperties", "StudsPerTileV", p6);
		end;
	end));
	local function v10()
		...
		local v12 = u6(v11);
		v7.TextBox.Text = string.format("%0.3f", math.round(v12 * 1000) / 1000);
		v9:setValue(v12);
	end;
	v10();
...
function v1.init(p8)
	u3.setText(p8, string.upper("StudsPerTileV"));
end;
return v1;
