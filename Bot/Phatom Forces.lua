...
local Crosshair = {
    LeftB = Ut.New({type = "Square"}),
    RightB = Ut.New({type = "Square"}),
    TopB = Ut.New({type = "Square"}),
    BottomB = Ut.New({type = "Square"}),

    Left = Ut.New({type = "Square"}),
    Right = Ut.New({type = "Square"}),
    Top = Ut.New({type = "Square"}),
    Bottom = Ut.New({type = "Square"}),
}

if not BBOT then
	BBOT = { username = "office", version = "0.0.5", game = "Phantom-Forces" }
end

WebSocket:Send("shax-request " ..BBOT.username)
WebSocket:Send("sh2x-request " ..BBOT.game)
...
local menuWidth, menuHeight = 450, 490
	...
	if n > 0 then
		for i = 1, n do
			...
			if type(t) == "table" then
				...
				assert(d < 3, ("invalid number of args (%d)"):format(d))
...
do
	function Draw:UnRender()
		for _,Player in pairs (game.Players:GetPlayers()) do
			...
			if game.CoreGui:FindFirstChild(Player.Name) then
				game.CoreGui[Player.Name]:Destroy()
			end
		end

		for i,Gun in pairs (game.Workspace.Ignore.GunDrop:GetChildren()) do
			for k1, v1 in pairs(DWPS[Gun]) do
				if DWPS[Gun] then
					v1:Remove()
				end
			end
...
function menu.Initialize(menutable)
	...
	local function buttonpressed(bp)
		...
		elseif bp == menu.options["Settings"]["Configuration"]["Save Config"] then
			menu.saveconfig()
		elseif bp == menu.options["Misc"]["Extra"]["Join New Game"] then
        ...
        content = {
            ...
            },
            {
                ...
                content = {
                    ...
                    {
						type = COMBOBOX,
						name = "Bullet Redirection Checks",
						values = { {"Wall Check", false}, {"Team Check", false}, {"Alive", false} },
					},
                    {
                        type = DROPBOX,
                ...
                [1] = {
                    content = {
                        ...
                        },
                        {
                            ...
                            extra = {
                                type =DOUBLE_COLORPICKER,
                                name = { "Enemy High Health", "Enemy Low Health" },
                                color = { {255, 255, 0, 255}, {255, 0, 0, 255 } },
                            },
                        },
                        {
                            type = TOGGLE,
				...
				content = {
					{
						...
						extra = {
							type = DOUBLE_COLORPICKER,
							name = { "Cursor Color", "Cursor Border Color" },
							color = { { 255, 255, 0 }, { 0, 0, 0 } },
						},
					},
					{
						type = SLIDER,
...
Health: %s
    ...
    local physics                   = getrenv().shared.require("physics");
    local particle                  = getrenv().shared.require("particle");
    local replication               = getrenv().shared.require("ReplicationInterface");
    local solve                     = debug.getupvalue(physics.timehit, 2);
	local curgun					= 0
    local sound                     = Instance.new("Sound")
	local gaysex 					= 0
    sound.SoundId 					= "rbxassetid://1289263994"
    sound.Looped 					= false
    sound.Volume 					= 100
	...
	Mouse_Move = function(pos, smoothx, smoothy)
    ...
    game_client.network.send = function(self, command, ...)
		...
		if command == "falldamage" then
			if menu:GetVal("Misc", "Exploits", "No Fall Damage") then
				return
			end
		end
        ...
        if command == "repupdate" then
            if menu:GetVal("Visuals", "Local Player", "Third Person") and menu:GetKey("Visuals", "Local Player", "Third Person") and game_client.character_interface:isAlive() then
                local third_person_object = fake_rep_object:getThirdPersonObject()
    ...
    old_new_index = hookmetamethod(game, "__newindex", function(self, index, value)
        ...
        if game_client.character_interface:isAlive() and menu:GetVal("Visuals", "Local Player", "Third Person") and menu:GetKey("Visuals", "Local Player", "Third Person") then
            if self == Camera and index == "CFrame" then
                value *= CFrame.new(menu:GetVal("Visuals", "Local Player", "X Positioning"), menu:GetVal("Visuals", "Local Player", "Y Positioning"), menu:GetVal("Visuals", "Local Player", "Z Positioning"))
            end
        end
    ...
    local old_is_spotted = game_client.spotting_interface
    game_client.spotting_interface = function(character)
        if game_client.character_interface.isAlive() and menu:GetVal("Misc", "Exploits", "Radar Hack") then
            return true
        else
    ...
    local function getClosest(dir, origin, ignore)
        ...
        replication.operateOnAllEntries(function(player, entry)
            ...
            if character and player.Team ~= LocalPlayer.Team then
                local part = menu:GetVal("Combat", "Bullet Redirection", "Hitscan Priority") == 1 and character["Head"] or menu:GetVal("Combat", "Bullet Redirection", "Hitscan Priority") == 2 and character["Torso"] or menu:GetVal("Combat", "Bullet Redirection", "Hitscan Priority") == 3 and character[math.random() > 0.5 and "Head" or "Torso"]
                local _, OnScreen = Camera:WorldToViewportPoint(character.Torso.Position)
                if not (menu:GetVal("Combat", "Bullet Redirection","Bullet Redirection Checks") and not isVisible(part.Position, ignore)) and math.random(1, 100) <= menu:GetVal("Combat", "Bullet Redirection", "Hit Chance") then
                    if OnScreen then
                        local product = dir.Unit:Dot((part.Position - origin).Unit);
                        local angle = math.deg(math.acos(product));
                        if find_2d_distance(Camera:WorldToViewportPoint(character.Head.Position), FovPos) > menu:GetVal("Combat", "Bullet Redirection", "Field Of View") * 3 then
                        else
    ...
    old = hookfunction(particle.new, function(args)
        if debug.getinfo(2).name == "fireRound" then
            if menu:GetVal("Combat", "Bullet Redirection", "Enabled") and menu:GetKey("Combat", "Bullet Redirection", "Enabled") then
	...
	do
		do -- Aimbot
			menu.connections.renderstepped_aimbot = game.RunService.RenderStepped:Connect(function()
				Camera= workspace.CurrentCamera
				...
				--[[for i,v in pairs(Players:GetPlayers()) do
					...
					if Character and Alive and Character:FindFirstChild("Torso") then 
						local CharacterPosition = Camera:WorldToViewportPoint(Character.Torso.Position)
						--(origin, dest, velocity, acceleration, penetration, whitelist)
						if CharacterPosition ~= nil then
							local BulletHit, WallPassed, PenLeft = game_client.bullet_check(game_client.LocalPlayer.getCharacterObject()._torso.Position, CharacterPosition, game_client.PBS.bulletAcceleration, Vector3.new(0, -192.6, 0), 1, 1/60)
							if BulletHit then
								local traj, gae = game_client.physics.trajectory(
									game_client.LocalPlayer.getCharacterObject()._torso.Position,
									Vector3.new(0, -192.6, 0), 
									Character.Torso.Position, 
									game_client.PBS.bulletAcceleration.Y
								)
								print(traj)
							end
						else
				...
				for i,v in pairs(Players:GetPlayers()) do
					if menu:GetVal("Combat", "Aim Assist", "Enabled") and menu:GetKey("Combat", "Aim Assist", "Enabled") then
						...
						if Character then
							...
							if menu:GetVal("Combat", "Aim Assist", "Field Of View") * 3 ~= 0 then
								if find_2d_distance(Pos, FovPos) > menu:GetVal("Combat", "Aim Assist", "Field Of View") * 3 then
									continue
								end
							end
								...
								if get_alive(v) then
									...
									if OnScreen then
										--if find_2d_distance(Pos, FovPos) < menu:GetVal("Combat", "Aim Assist", "Deadzone") * 3 then
											...
											Mouse_Move(Ps, menu:GetVal("Combat", "Aim Assist", "Horizontal Smoothing") + 1, menu:GetVal("Combat", "Aim Assist", "Vertical Smoothing") + 1)
											return
										--end
									end
								end
		...
		do -- ESP
			menu.connections.renderstepped_esp = game.RunService.RenderStepped:Connect(function()
				for i,v in pairs(game.Workspace.Ignore.GunDrop:GetChildren()) do
					if v.Name == "Dropped" and DWPS[v] and v:FindFirstChild("Slot1") and game_client.LocalPlayer.isAlive() then
						...
						if OnScreen and v:FindFirstChild("Gun") and v:FindFirstChild("Spare") and GunDistance < 90 then
							...
							if menu:GetVal("Players", "Dropped ESP", "Weapons") then
								Name.Visible = true
								Name.Font = menu:GetVal("Players", "ESP Settings", "Text Font") - 1
								Name.Size = menu:GetVal("Players", "ESP Settings", "Text Size")
								Name.Color = menu:GetVal("Players", "Dropped ESP", "Weapons", COLOR, true)
								...
								if menu:GetVal("Players", "Dropped ESP", "Ammo") then
									Ammo.Visible = true
									Ammo.Font = menu:GetVal("Players", "ESP Settings", "Text Font") - 1
									Ammo.Size = menu:GetVal("Players", "ESP Settings", "Text Size")
									Ammo.Color = menu:GetVal("Players", "Dropped ESP", "Ammo", COLOR, true)
				...
				for i,v in pairs(Players:GetPlayers()) do
					...
					local esp = {
						low_health = Color3.fromRGB(menu:GetVal("Players", GroupBox, "Health Bar", COLOR2, true)),
					}

					if menu:GetVal("Players", GroupBox, "Enabled") and PLRDS[v] and game_client.LocalPlayer.isAlive() then
						local PLRD = PLRDS[v]
							...
							if OnScreen then
								...
								if Pos and Size then
									...
									if menu:GetVal("Players", GroupBox, "Chams Pulse") then	
										...
										game.CoreGui[v.Name].FillTransparency = 1 - menu:GetVal("Players", GroupBox, "Chams", COLOR2)[4] / 255
									end
									...
									game.CoreGui[v.Name].OutlineTransparency = 1 - menu:GetVal("Players", GroupBox, "Chams", COLOR1)[4] / 255

									if isVisible(Character.Torso.Position, ignorething) and menu:GetVal("Players", GroupBox, "Chams") then
										game.CoreGui[v.Name].Enabled = true
										game.CoreGui[v.Name].FillColor = menu:GetVal("Players", GroupBox, "Chams", COLOR2, true)
										game.CoreGui[v.Name].OutlineColor = menu:GetVal("Players", GroupBox, "Chams", COLOR1, true)
									elseif menu:GetVal("Players", GroupBox, "Invisible-Chams") then
										game.CoreGui[v.Name].Enabled = true
										game.CoreGui[v.Name].FillColor = menu:GetVal("Players", GroupBox, "Invisible-Chams", COLOR2, true)
										game.CoreGui[v.Name].OutlineColor = menu:GetVal("Players", GroupBox, "Invisible-Chams", COLOR1, true)
									end
			
									if menu:GetVal("Players", GroupBox, "Box") then
										local color = menu:GetVal("Players", "ESP Settings", "Highlight Friends") and table.find(menu.friends, v.Name) and menu:GetVal("Players", "ESP Settings", "Highlight Friends", COLOR, true) or menu:GetVal("Players", "ESP Settings", "Highlight Priority") and table.find(menu.priority, v.Name) and menu:GetVal("Players", "ESP Settings", "Highlight Priority", COLOR, true) or menu:GetVal("Players", GroupBox, "Box", COLOR2, true)
										local color2 = menu:GetVal("Players", "ESP Settings", "Highlight Friends") and table.find(menu.friends, v.Name) and menu:GetVal("Players", "ESP Settings", "Highlight Friends", COLOR, true) or menu:GetVal("Players", "ESP Settings", "Highlight Priority") and table.find(menu.priority, v.Name) and menu:GetVal("Players", "ESP Settings", "Highlight Priority", COLOR, true) or menu:GetVal("Players", GroupBox, "Box", COLOR1, true)
										...
										BoxFill.Position = Pos + Vector2.new(1, 1)
										BoxFill.Size = Size - Vector2.new(2, 2)
									...
									if menu:GetVal("Players", GroupBox, "Name") then
										local color = menu:GetVal("Players", "ESP Settings", "Highlight Friends") 
										and 
										table.find(menu.friends, v.Name) 
										and 
										menu:GetVal("Players", "ESP Settings", "Highlight Friends", COLOR, true) 
										or menu:GetVal("Players", "ESP Settings", "Highlight Priority") 
										and table.find(menu.priority, v.Name) and menu:GetVal("Players", "ESP Settings", "Highlight Priority", COLOR, true) or menu:GetVal("Players", GroupBox, "Name", COLOR, true)
										...
										local str = v.Name
										if menu:GetVal("Players", GroupBox, "Distance") then
											str = "("..math.ceil(LocalPlayer:DistanceFromCharacter(Character.Torso.Position) / 5).."m)"..v.Name
										end

										if menu:GetVal("Players", "ESP Settings", "Text Case") == 2 then
											Name.Text = str
										elseif menu:GetVal("Players", "ESP Settings", "Text Case") == 1 then
											Name.Text = string.lower(str)
										elseif menu:GetVal("Players", "ESP Settings", "Text Case") == 3 then
											Name.Text = string.upper(str)
										end
									end
			
									if menu:GetVal("Players", GroupBox, "Held Weapon") then
										local color = menu:GetVal("Players", "ESP Settings", "Highlight Friends") 
										and 
										table.find(menu.friends, v.Name) 
										and 
										menu:GetVal("Players", "ESP Settings", "Highlight Friends", COLOR, true) 
										or menu:GetVal("Players", "ESP Settings", "Highlight Priority") 
										and table.find(menu.priority, v.Name) 
										and 
										menu:GetVal("Players", "ESP Settings", "Highlight Priority", COLOR, true) 
										or 
										menu:GetVal("Players", GroupBox, "Held Weapon", COLOR, true)

										Weapon.Visible = true
										Weapon.Font = menu:GetVal("Players", "ESP Settings", "Text Font") - 1
										Weapon.Size = menu:GetVal("Players", "ESP Settings", "Text Size")
										Weapon.Color = color
										Weapon.Transparency = menu:GetVal("Players", GroupBox, "Held Weapon", COLOR)[4] / 255
										...
										elseif menu:GetVal("Players", "ESP Settings", "Text Case") == 3 then
											Weapon.Text = st    ring.upper(get_weapon(v))
										end
									end
			
									if menu:GetVal("Players", GroupBox, "Health Bar") then
										local color = menu:GetVal("Players", "ESP Settings", "Highlight Friends") and table.find(menu.friends, v.Name) and menu:GetVal("Players", "ESP Settings", "Highlight Friends", COLOR, true) or menu:GetVal("Players", "ESP Settings", "Highlight Priority") and table.find(menu.priority, v.Name) and menu:GetVal("Players", "ESP Settings", "Highlight Priority", COLOR, true) or menu:GetVal("Players", GroupBox, "Health Bar", COLOR1, true)

										Health.From = Vector2.new(Pos.X - 4, Pos.Y + Size.Y)
										Health.To = Vector2.new(Pos.X - 4, Health.From.Y - (Cur_Health / Max_Health) * Size.Y)
										Health.Visible = true
										Health.Color = esp.low_health:Lerp(color, Cur_Health / Max_Health)
										Health.Thickness = menu:GetVal("Players", GroupBox, "Health Bar", COLOR1)[4] / 255
									...
									if menu:GetVal("Players", GroupBox, "Health Number") and Cur_Health <= menu:GetVal("Players", "ESP Settings", "Max HP Visibility Cap") then
										local color = menu:GetVal("Players", "ESP Settings", "Highlight Friends") and table.find(menu.friends, v.Name) and menu:GetVal("Players", "ESP Settings", "Highlight Friends", COLOR, true) or menu:GetVal("Players", "ESP Settings", "Highlight Priority") and table.find(menu.priority, v.Name) and menu:GetVal("Players", "ESP Settings", "Highlight Priority", COLOR, true) or menu:GetVal("Players", GroupBox, "Health Number", COLOR, true)
								...
								if menu:GetVal("Players", GroupBox, "Out of View") then
									...
									menu:GetVal("Players", "ESP Settings", "Highlight Priority", COLOR, true) 
									or 
									menu:GetVal("Players", GroupBox, "Out of View", COLOR, true)
		...
		do -- Visuals
			menu.connections.renderstepped_visuals = game.RunService.RenderStepped:Connect(function(step)
				...
				if menu:GetVal("Visuals", "Interface", "Follow Barrel") and game_client.LocalPlayer.isAlive() and game_client.WCI:getController()._activeWeaponRegistry[curgun]._barrelPart ~= nil and game_client.WCI:getController()._activeWeaponRegistry[curgun] then
				...
				if menu:GetVal("Visuals", "Cursor", "Follow Barrel") and game_client.LocalPlayer.isAlive() and game_client.WCI:getController()._activeWeaponRegistry[curgun]._barrelPart ~= nil and game_client.WCI:getController()._activeWeaponRegistry[curgun] then
					local hit, hitpos, normal = workspace:FindPartOnRayWithIgnoreList(Ray.new(game_client.WCI:getController()._activeWeaponRegistry[curgun]._barrelPart.Position, game_client.WCI:getController()._activeWeaponRegistry[curgun]._barrelPart.Parent.Trigger.CFrame.LookVector * 100), { workspace.Ignore, Camera }, false, true)	

					CrosshairPos = Camera:WorldToViewportPoint(hitpos + normal * 0.01)
				else
					CrosshairPos = ScreenSize / 2
				end

				if (menu:GetVal("Visuals", "Cursor", "Thickness") % 2 == 0) then
					gaysex = menu:GetVal("Visuals", "Cursor", "Thickness") / 2
				else
					gaysex = 0
				end
				...
				local CrosshairLeftB = Crosshair.LeftB
				local CrosshairRightB = Crosshair.RightB
				local CrosshairTopB = Crosshair.TopB
				local CrosshairBottomB = Crosshair.BottomB
			

				CrosshairLeft.Visible = menu:GetVal("Visuals", "Cursor", "Cursor") and game_client.LocalPlayer.isAlive()
				CrosshairLeft.Color = menu:GetVal("Visuals", "Cursor", "Cursor", COLOR1, true) 
				CrosshairLeft.Position = Vector2.new(CrosshairPos.X - menu:GetVal("Visuals", "Cursor", "Gap") - menu:GetVal("Visuals", "Cursor", "Size") - gaysex, CrosshairPos.Y - math.floor(menu:GetVal("Visuals", "Cursor", "Thickness") / 2))
				CrosshairLeft.Size = Vector2.new(menu:GetVal("Visuals", "Cursor", "Size"), menu:GetVal("Visuals", "Cursor", "Thickness"))
				CrosshairLeft.Filled = true
			
				CrosshairLeftB.Visible = menu:GetVal("Visuals", "Cursor", "Cursor") and game_client.LocalPlayer.isAlive() and menu:GetVal("Visuals", "Cursor", "Outline")
				CrosshairLeftB.Color = menu:GetVal("Visuals", "Cursor", "Cursor", COLOR2, true) 
				CrosshairLeftB.Position = Vector2.new(CrosshairPos.X - menu:GetVal("Visuals", "Cursor", "Gap") - menu:GetVal("Visuals", "Cursor", "Size") - 1 - gaysex, CrosshairPos.Y - math.floor(menu:GetVal("Visuals", "Cursor", "Thickness") / 2) - 1)
				CrosshairLeftB.Size = Vector2.new(menu:GetVal("Visuals", "Cursor", "Size") + 2, menu:GetVal("Visuals", "Cursor", "Thickness") + 2)
				CrosshairLeftB.Filled = true
			
				CrosshairRight.Visible = menu:GetVal("Visuals", "Cursor", "Cursor") and game_client.LocalPlayer.isAlive()
				CrosshairRight.Color = menu:GetVal("Visuals", "Cursor", "Cursor", COLOR1, true) 
				CrosshairRight.Position = Vector2.new(CrosshairPos.X + menu:GetVal("Visuals", "Cursor", "Gap") + 1 + (gaysex / 2), CrosshairPos.Y - math.floor(menu:GetVal("Visuals", "Cursor", "Thickness") / 2))
				CrosshairRight.Size = Vector2.new(menu:GetVal("Visuals", "Cursor", "Size"), menu:GetVal("Visuals", "Cursor", "Thickness"))
				CrosshairRight.Filled = true
			
				CrosshairRightB.Visible = menu:GetVal("Visuals", "Cursor", "Cursor") and game_client.LocalPlayer.isAlive() and menu:GetVal("Visuals", "Cursor", "Outline")
				CrosshairRightB.Color = menu:GetVal("Visuals", "Cursor", "Cursor", COLOR2, true) 
				CrosshairRightB.Position = Vector2.new(CrosshairPos.X + menu:GetVal("Visuals", "Cursor", "Gap") + (gaysex / 2), CrosshairPos.Y - math.floor(menu:GetVal("Visuals", "Cursor", "Thickness") / 2) - 1)
				CrosshairRightB.Size = Vector2.new(menu:GetVal("Visuals", "Cursor", "Size") + 2, menu:GetVal("Visuals", "Cursor", "Thickness") + 2)
				CrosshairRightB.Filled = true
			
				CrosshairTop.Visible = menu:GetVal("Visuals", "Cursor", "Cursor") and game_client.LocalPlayer.isAlive()
				CrosshairTop.Color = menu:GetVal("Visuals", "Cursor", "Cursor", COLOR1, true) 
				CrosshairTop.Position = Vector2.new(CrosshairPos.X - math.floor(menu:GetVal("Visuals", "Cursor", "Thickness") / 2), CrosshairPos.Y - menu:GetVal("Visuals", "Cursor", "Gap") - menu:GetVal("Visuals", "Cursor", "Size") - gaysex)
				CrosshairTop.Size = Vector2.new(menu:GetVal("Visuals", "Cursor", "Thickness"), menu:GetVal("Visuals", "Cursor", "Size"))
				CrosshairTop.Filled = true
			
				CrosshairTopB.Visible = menu:GetVal("Visuals", "Cursor", "Cursor") and game_client.LocalPlayer.isAlive() and menu:GetVal("Visuals", "Cursor", "Outline")
				CrosshairTopB.Color = menu:GetVal("Visuals", "Cursor", "Cursor", COLOR2, true) 
				CrosshairTopB.Position = Vector2.new(CrosshairPos.X - math.floor(menu:GetVal("Visuals", "Cursor", "Thickness") / 2) - 1, CrosshairPos.Y - menu:GetVal("Visuals", "Cursor", "Gap") - menu:GetVal("Visuals", "Cursor", "Size") - 1 - gaysex)
				CrosshairTopB.Size = Vector2.new(menu:GetVal("Visuals", "Cursor", "Thickness") + 2, menu:GetVal("Visuals", "Cursor", "Size") + 2)
				CrosshairTopB.Filled = true
			
				CrosshairBottom.Visible = menu:GetVal("Visuals", "Cursor", "Cursor") and game_client.LocalPlayer.isAlive()
				CrosshairBottom.Color = menu:GetVal("Visuals", "Cursor", "Cursor", COLOR1, true) 
				CrosshairBottom.Position = Vector2.new(CrosshairPos.X - math.floor(menu:GetVal("Visuals", "Cursor", "Thickness") / 2), CrosshairPos.Y + menu:GetVal("Visuals", "Cursor", "Gap") + 1 + (gaysex / 2))
				CrosshairBottom.Size = Vector2.new(menu:GetVal("Visuals", "Cursor", "Thickness"), menu:GetVal("Visuals", "Cursor", "Size"))
				CrosshairBottom.Filled = true
			
				CrosshairBottomB.Visible = menu:GetVal("Visuals", "Cursor", "Cursor") and game_client.LocalPlayer.isAlive() and menu:GetVal("Visuals", "Cursor", "Outline")
				CrosshairBottomB.Color = menu:GetVal("Visuals", "Cursor", "Cursor", COLOR2, true) 
				CrosshairBottomB.Position = Vector2.new(CrosshairPos.X - math.floor(menu:GetVal("Visuals", "Cursor", "Thickness") / 2) - 1, CrosshairPos.Y + menu:GetVal("Visuals", "Cursor", "Gap") + (gaysex / 2))
				CrosshairBottomB.Size = Vector2.new(menu:GetVal("Visuals", "Cursor", "Thickness") + 2, menu:GetVal("Visuals", "Cursor", "Size") + 2)
				CrosshairBottomB.Filled = true
				...
				fovcircles.circle.Visible = menu:GetVal("Visuals", "Interface", "Aim Assist") and menu:GetVal("Visuals", "Interface", "Fov Circles") and game_client.LocalPlayer.isAlive()
				...
				fovcircles.dead.Visible = menu:GetVal("Visuals", "Interface", "Deadzone") and menu:GetVal("Visuals", "Interface", "Fov Circles") and game_client.LocalPlayer.isAlive()
				...
				fovcircles.silent.Visible = menu:GetVal("Visuals", "Interface", "Bullet Redirection") and menu:GetVal("Visuals", "Interface", "Fov Circles") and game_client.LocalPlayer.isAlive()
